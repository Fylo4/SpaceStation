...
export class DigitOnlyDirective implements OnChanges {
  ...
  onPaste(event: any): void {
    if (this.allowPaste === true) {
      ...
      if ((window as Record<string, any>)['clipboardData']) {
        // Browser is IE
        pastedInput = (window as Record<string, any>)['clipboardData'].getData('text');
  ...
  private pasteData(pastedContent: string): void {
    ...
    if (!pasted) {
      if (this.inputElement.setRangeText) {
        ...
        if (typeof (window as Record<string, any>)['InstallTrigger'] !== 'undefined') {
  ...
  private sanitizeInput(input: string): string {
    ...
    if (maxLength > 0) {
      ...
      const allowedLength =
        maxLength - this.inputElement.value.length + (result.includes(`${this.negativeSign}`) ? 1 : 0);
